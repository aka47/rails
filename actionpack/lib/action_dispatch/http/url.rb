...
module ActionDispatch
  module Http
    class URL
        ...
        private
          ...
          def normalize_host(_host, options)
            ...
            if subdomain == true
              ...
              host << ActionDispatch::Http::URL.extract_subdomains(_host, tld_length).join(".")
            elsif subdomain
              host << subdomain.to_param
            end
            host << "." unless host.empty?
            host << (domain || ActionDispatch::Http::URL.extract_domain(_host, tld_length))
            host
          end
      ...
      def initialize(uri)
        if ipv6 = uri.match(IP6_REGEX)
          @core_uri = ::URI::Generic.build2(scheme: ipv6[1], host: ipv6[2], port: ipv6[3], path: ipv6[4])
        else
          @core_uri = ::URI.parse(uri)
        end
      end
      ...
      def subdomain(tld_length = ActionDispatch::Http::URL.tld_length)
        ActionDispatch::Http::URL.extract_subdomains(host, tld_length).join(".")
      end

      def extract_domain_from(host, tld_length)
        ActionDispatch::Http::URL.extract_domain_from(host, tld_length)
      end
